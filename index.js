...
const token =
  "a3bb803c27770ea3a0082be2b77c328eE86E433926A9FF64D7223EFB32D698699962043D";
...
async function getUnits() {
  ...
  for (const id of ids) {
    try {
      ...
      const time = timeObj.toISOString().replace(/[TZ]/g, "");
      const fechaString = time.substring(0, 10);
      const horaString = time.substring(10);
      const fecha = new Date(fechaString);

      const timeUTC = `${fechaString} ${horaString}`;
      ...
      await axios
        .post(apiSitrackUrl, dataSitrack)
        .then((response) => {
          console.log("Respuesta del servidor", response.data.status);
          const statusSitrack = response.data.status;
          const data = {
            id: id,
            imei: imei,
            name: name,
        ...
        .catch((error) => {
          ...
          const statusSitrack = error.message;
          const data = {
            id: id,
            imei: imei,
            name: name,
...
function reemplazarComillas(objeto) {
  // const objetoModificado = {};
  // for (const clave in objeto) {
  //   if (objeto.hasOwnProperty(clave)) {
  //     objetoModificado[clave] = objeto[clave].replace(/'/g, '"');
  //   }
  // }
  // return objetoModificado;
  const objetoModificado = JSON.parse(
